...
from app.extraction.algorithms.projection import (
    build_vertical_projection,
    build_horizontal_projection,
    dominant_axis,
)
from app.extraction.algorithms.manhattan import classify_layout
...
def detect_page_layout(page: Dict) -> Dict:
    ...
    # Two-column override. If there is a clear vertical gutter down the middle
    # of the page with text on both sides, treat it as multi_col and read the
    # columns first (v_major). Without this, such pages were classified "mixed"
    # and the axis scorer -- which counts the many horizontal line-gaps -- often
    # picked h_major, cutting horizontally across both columns and interleaving
    # them line by line.
    if layout_type in ("mixed", "single_col") and _detect_central_gutter(blocks, page_width):
        logger.info(
            f"[Detector] Page {page_number} -- central gutter detected, "
            f"overriding to multi_col / v_major"
        )
...
def _detect_central_gutter(
    ...
    """Return True if there is a whitespace gutter down the middle with text on
    both sides -- the signature of two columns.

    Full-width blocks (titles, spanning headings, footnotes) are excluded so
    they don't fill the gutter and hide it.
    """
    if page_width <= 0:
        return False
    ...
    profile = [0.0] * n
    for b in narrow:
        length = len(b.get("text", "").strip())
        start = int(b["bbox"][0] / bucket)
        end = int(b["bbox"][2] / bucket)
    ...
    if best_run < max(1, int(n * min_width_ratio)):
        return False

    return sum(profile[:lo]) > 0 and sum(profile[hi:]) > 0
