...
def _local_strategy(
    ...
    """
    Determine the dominant reading strategy (h_major or v_major)
    for a specific sub-region independently.

    This is the key fix for mixed layouts -- instead of inheriting
    the parent page strategy, each region evaluates its own
    projection profiles locally.

    Example: a page with a single-column header and a two-column body
    will have:
      - Top region (header): strong horizontal gaps → h_major
      - Bottom region (body): strong vertical gap in middle → v_major
    """
    if not blocks:
        return "h_major"

    v_profile = _vertical_projection(blocks, x0, x1, resolution)
    h_profile = _horizontal_projection(blocks, y0, y1, resolution)

    v_valleys = _find_cut_valleys(v_profile, threshold_ratio, min_width)
    h_valleys = _find_cut_valleys(h_profile, threshold_ratio, min_width)

    v_widths = [e - s + 1 for s, e in v_valleys]
    h_widths = [e - s + 1 for s, e in h_valleys]

    avg_v_gap = statistics.mean(v_widths) if v_widths else 0
    avg_h_gap = statistics.mean(h_widths) if h_widths else 0

    v_score = avg_v_gap * len(v_valleys)
    h_score = avg_h_gap * len(h_valleys)

    strategy = "v_major" if v_score >= h_score else "h_major"

    logger.debug(
        f"[XY-Cut] local_strategy region=({x0:.1f},{y0:.1f},{x1:.1f},{y1:.1f}) "
        f"v_score={v_score:.2f} h_score={h_score:.2f} → {strategy}"
    )
...
def xy_cut(
    ...
    At each recursion level, the strategy is RE-EVALUATED locally
    for the current sub-region rather than inherited from the parent.
    This correctly handles mixed layouts such as:
    ...
    logger.debug(
        ...
        f"blocks={len(region_blocks)} inherited_strategy={strategy}"
    )
    ...
    if depth >= max_depth or len(region_blocks) <= 1:
        ...
        return [{"blocks": region_blocks, "bbox": (x0, y0, x1, y1), "depth": depth, "strategy": strategy}]

    # ── Local strategy re-evaluation ─────────────────────────────────────────
    # At depth 0 we use the page-level strategy from detector.
    # At depth > 0 we re-evaluate locally -- this is what fixes mixed layouts.
    if depth > 0:
        local_strategy = _local_strategy(
            region_blocks, x0, y0, x1, y1,
            resolution=resolution,
            threshold_ratio=threshold_ratio,
            min_width=min_valley_width,
        )
        if local_strategy != strategy:
            logger.info(
                f"[XY-Cut] depth={depth} -- strategy override: "
                f"{strategy} → {local_strategy} "
                f"for region ({x0:.1f},{y0:.1f},{x1:.1f},{y1:.1f})"
            )
...
def flatten_regions_to_blocks(regions: List[Dict], strategy: str = "v_major") -> List[dict]:
    ...
    Key improvement: each leaf region uses its OWN locally detected strategy
    for sorting blocks within it, not the global page strategy.
    This ensures blocks within a column region sort top-to-bottom,
    and blocks within a row region sort left-to-right, independently.
    """
    ordered_blocks = []
    ...
    for region in regions:
        ...
        # Use region's own local strategy if available, else fall back to global
        local_strategy = region.get("strategy", strategy)

        if local_strategy == "v_major":
